...
if __name__ == '__main__':
    ...
    plot_scatter.py file.data abs "x label" "y label" A B C D
    ...
    USAGE = '''USAGE: python plot_scatter.py <data file> op xlabel ylabel Class [Class..].
            ...
            e.g. python plot_scatter.py file.data abs x y A B C D, will compute abs(X1-X2)'''
    ...
    x = range(len(threads_array))
    #markers = [".", ",", "o", "v", "^","<", ">", "1", "2", "3", "4", "8", "s", "p", "P", "*", "h", "H", "+", "x", "X", "D", "d", "|", "_"]
    markers = ["*", "h", "+", "x", "^", "o", "<", "v", ">", "1", "2", "3", "4", "8"]
    benchmark_labels = benchmarks.keys()

    # labels
    xticks = threads_array

    format_fn = lambda tick_val, tick_pos: xticks[int(tick_val)] if int(tick_val) in range(len(xticks)) else ''
    ...
    if len(classes) == 1:
        ...
        #plt.scatter(data[cl], labels=xticks, showmeans=True, meanline=False, medianprops=medianprops, meanprops=meanpointprops)
        for i in range(len(benchmark_labels)):
            plt.scatter(x, data[cl][i][:], c="k", alpha=1, s=70, edgecolors='none', label=benchmark_labels[i], marker=markers[i])

        plt.set_major_formatter(FuncFormatter(format_fn))
        plt.set_major_locator(MaxNLocator(integer=True))
        # for name, line_list in b.iteritems():
        #     for line in line_list:
        #         line.set_color('k')
        plt.legend(loc='upper left', framealpha=1, scatterpoints=1)#, bbox_to_anchor=(1.8, 1))
        ...
        for i in range(len(classes)):
            ...
            for j in range(len(benchmark_labels)):
                axes[i].scatter(x, data[cl][j][:], c="k", alpha=1, s=70, edgecolors='none', label=benchmark_labels[j], marker=markers[j])

            # for name, line_list in b.iteritems():
            #     for line in line_list:
            #         line.set_color('k')
            axes[i].xaxis.set_major_formatter(FuncFormatter(format_fn))
            axes[i].xaxis.set_major_locator(MaxNLocator(integer=True))
            axes[i].set_xlabel(x_label)
            axes[i].minorticks_on()

            if i == 0:
                #axes[i].legend(loc='upper left', framealpha=1)#, bbox_to_anchor=(1.8, 1))
                #axes[i].legend(loc='lower right', framealpha=1, scatterpoints=1, bbox_to_anchor=(1.2, 0.0))
                axes[i].legend(loc='upper left', framealpha=1, scatterpoints=1)# bbox_to_anchor=(0.0, 0.0))
